...
local function leftButtonEvent( event )
	if event.phase == "ended" then
		...
		if currScene then
			...
			if composer.getScene(Menu) then
			composer.showOverlay( Menu, { isModal=true, time=250, effect="fromLeft" } )
			else
	            native.showAlert("classes/ui.lua", "Please set a filename for Menu scene", { "OK" } )
			end
		end
	end
	return true
